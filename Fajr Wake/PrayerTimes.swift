...
open class PrayerTimes {
    ...
    // return prayer times for a given date
    open func getPrayerTimes(_ calendar: Calendar, date: Date, latitude: Double, longitude: Double, tZone: Double) -> [String: String] {
        let year = ((calendar as NSCalendar).component(NSCalendar.Unit.year, from: date))
        let month = ((calendar as NSCalendar).component(NSCalendar.Unit.month, from: date))
    ...
    open func setCustomParams(_ params: [Double]) { // FIXME: got rid of var
        var params = params
    ...
    func adjustHighLatTimes(_ times: [Double]) -> [Double]{ // FIXME: Got rid of var
        var times = times
